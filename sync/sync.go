...
func StartPullSync(fireflyClient *firefly.FireflyClient, bunqClient *bunq.BunqClient, log *logrus.Logger) error {
	arguments := os.Args
	var date time.Time
	if len(arguments) >= 2 {
		var err error
		...
		for processTransactions {
			...
			for _, payment := range payments {
				...
				if date.Compare(payment.Created.Time) >= 1 {
					processTransactions = false
					paymentLogger.Debug("Received payment too far in the past, stop processing")
					continue
				}

				paymentLogger.Debug("Start processing payment")
				...
				if transactions.Meta.Pagination.Total > 0 {
					...
					paymentLogger.Debug("Payment already in firefly, skipping payment")
					continue
				}
...
func StartPushSync() error {
	// TODO: Start webserver
	// TODO: Add callback in bunq
	// TODO: Listen for webhooks from bunq
	// TODO: Store transactions in firefly
